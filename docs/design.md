...
2. pid, API socket, virtiofs socket, virtiofs helper pid 경로를 만든다.
...
- 현재 구현된 manifest-managed network path는 단일 TAP NIC이며, future `passt` 기본 backend 방향은 [`adr/0001-passt-default-network-backend.md`](adr/0001-passt-default-network-backend.md)에 별도 ADR로 기록한다. ADR 내용은 구현 완료 전까지 현재 동작 설명으로 쓰지 않는다.
