...
`Hypervisor.Start()` 시작 gate는 모든 backend에서 `<node-root>/<node>/run/` symlink/non-directory를 거부하고, 같은 경로가 현재 `cvmm` manager/service uid 소유이며 mode가 `0700` 이내인지 확인한다. `passt` backend의 dedicated non-root service uid/gid, `--runas` 제한은 그 다음 start-time 검증으로 수행한다. `client`, `console`, `shutdown`도 socket/pid 접근 전에 같은 runtime directory owner/mode/symlink 검증을 수행한다. 따라서 운영 중 진단/정리 명령도 VM을 관리하는 service uid 또는 동일한 manager uid로 호출해야 한다.
...
2. 모든 backend에서 `run/` directory symlink/non-directory, owner/mode(`<=0700`) 검증
3. `passt` backend면 dedicated non-root service uid/gid와 `--runas` 제한 검증
...
- 모든 backend에서 `<node-root>/<node>/run/` symlink/non-directory는 거부되고, `start`는 manager/service uid 소유와 mode `<=0700`를 요구한다.
- `client`, `console`, `shutdown`은 socket/pid 접근 전에 같은 runtime path의 owner/mode/symlink 검증을 수행한다.
