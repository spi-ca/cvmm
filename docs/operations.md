...
- backend별 권한 준비
  - 기본 `passt`: dedicated non-root service user/group, 안전한 `<node-root>/<node>/run/` 소유권/권한
...
- `<node-root>/<node>/run/` 소유자가 실제 서비스 uid인지
...
- 기본 backend면 `passt.sock`, `passt.pid`가 있는지와 mode/owner가 기대값인지 확인
