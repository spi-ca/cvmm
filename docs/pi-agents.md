...
- `.pi/skills/run-screenfs-benchmarks/SKILL.md`: historical filename을 유지한 채 cvmm validation/measurement helper로 사용
...
- Go 코드/CLI/config builder: `go test ./...`
- YAML/OpenAPI manifest: local YAML parser로 touched file parse
- systemd unit (`contrib/cvmm@.service`): 가능하면 `systemd-analyze verify contrib/cvmm@.service`
- `.pi`/docs-only 변경: JSON parse, frontmatter spot-check, inventory listing, `git diff --check`
