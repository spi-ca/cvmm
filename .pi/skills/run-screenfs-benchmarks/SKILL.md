...
description: Historical skill name retained; run and record cvmm validation or measurement evidence for startup, orchestration, manifest, or service changes.
---

# Repo Validation and Measurement Helper

This skill keeps its historical folder name for compatibility, but its workflow is for the current cvmm repository.
...
- You need a compact evidence bundle for current behavior, not an unrelated ScreenFS benchmark run.
...
3. Prefer focused measurements around cvmm behavior (for example start/shutdown flows, command construction, manifest loading, or relevant tests) rather than synthetic FUSE benchmarks.
...
- Do not reuse ScreenFS/FUSE semantics, hidden-path rules, or benchmark claims in cvmm evidence.
